...
export default new Vuex.Store({
  ...
  actions: {
    async initialize(context) {
      try {
        const response = await RssApi.announcement.getAll();
    ...
    async addItem(context, item) {
      try {
        const response = await RssApi.announcement.add(item);
    ...
    async editItem(context, item) {
      try {
        const response = await RssApi.announcement.edit(item.id, item);
    ...
    async removeItem(context, id) {
      try {
        const response = await RssApi.announcement.delete(id);
